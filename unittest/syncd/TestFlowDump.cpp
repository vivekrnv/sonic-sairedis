...
TEST_F(FlowDumpTest, FlowDumpSerializer_SingleFlowEntry)
{
    ...
    FlowDumpSerializer& serializer = FlowDumpSerializer::getInstance();
    FlowDumpDataPtr result = serializer.serializeToJsonLines(0x1000000000001, 1, event_data);

    ASSERT_NE(result, nullptr);
    ASSERT_EQ(result->json_lines.size(), 1);

    nlohmann::json& json_line = result->json_lines[0];
...
TEST_F(FlowDumpTest, FlowDumpSerializer_MultipleFlowEntries)
{
    ...
    event_data[1].flow_entry.vnet_id = 2; // uint16_t, so use small values
    event_data[1].flow_entry.src_port = 54321;
    event_data[1].flow_entry.dst_port = 443;

    // Third flow entry - FINISHED event (should be skipped)
    event_data[2] = m_event_data;
    event_data[2].event_type = SAI_FLOW_BULK_GET_SESSION_EVENT_FINISHED;

    FlowDumpSerializer& serializer = FlowDumpSerializer::getInstance();
    FlowDumpDataPtr result = serializer.serializeToJsonLines(0x1000000000001, 3, event_data);

    ASSERT_NE(result, nullptr);
    // Should only have 2 entries (FINISHED event is skipped)
    ASSERT_EQ(result->json_lines.size(), 2);

    // Verify first entry
    EXPECT_EQ(result->json_lines[0]["vn"], 1);
    EXPECT_EQ(result->json_lines[0]["sp"], 12345);

    // Verify second entry
    EXPECT_EQ(result->json_lines[1]["vn"], 2);
    EXPECT_EQ(result->json_lines[1]["sp"], 54321);
    EXPECT_EQ(result->json_lines[1]["dp"], 443);
}

// Test FlowDumpSerializer with no FLOW_ENTRY events
TEST_F(FlowDumpTest, FlowDumpSerializer_NoFlowEntries)
{
    sai_flow_bulk_get_session_event_data_t event_data[1];
    event_data[0] = m_event_data;
    ...
    // Should return nullptr when there are no FLOW_ENTRY events
    ASSERT_EQ(result, nullptr);
}

// Test FlowDumpSerializer with null data
TEST_F(FlowDumpTest, FlowDumpSerializer_NullData)
{
    FlowDumpSerializer& serializer = FlowDumpSerializer::getInstance();
    FlowDumpDataPtr result = serializer.serializeToJsonLines(0x1000000000001, 1, nullptr);
...
TEST_F(FlowDumpTest, FlowDumpSerializer_WithAttributes)
{
    ...
    event_data[0].attr_count = 4;
    event_data[0].attr = attrs;
    ...
    EXPECT_EQ(json_line[attr_version_key], 12345);

    ASSERT_TRUE(json_line.contains(attr_unidirectional_key));
    EXPECT_EQ(json_line[attr_unidirectional_key], true);
...
TEST_F(FlowDumpTest, FlowDumpWriter_WithAttributes)
{
    ...
    FlowDumpSerializer& serializer = FlowDumpSerializer::getInstance();
    FlowDumpDataPtr data = serializer.serializeToJsonLines(0x5000000000001, 2, event_data);
    ...
    EXPECT_EQ(read_lines[0][attr_version_key1], 100);
    
    ASSERT_TRUE(read_lines[0].contains(attr_unidirectional_key));
    EXPECT_EQ(read_lines[0][attr_unidirectional_key], true);
    ...
    EXPECT_EQ(read_lines[1][attr_version_key2], 200);
